...
for name, df in all_data.items():
    ...
    # MultiIndex-Spalten abflachen, falls vorhanden
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)
    
    # Nur existierende Spalten mit mindestens einem Wert plotten
    possible_cols = ['Close', 'SMA20', 'SMA50', 'rsi']
    columns_to_plot = [col for col in possible_cols if col in df.columns and df[col].notna().any()]
