...
def compute_signals(df, rsi_len=14):
    ...
    # Prüfen, ob genug Daten für SMA vorhanden
    if len(df) < 20:
        return pd.DataFrame()
...
for name, df in all_data.items():
    ...
    if 'ATR' in last:
        atr_data = last['ATR']
        if isinstance(atr_data, (pd.Series, pd.DataFrame)):
            atr_data = atr_data.values[0]
        if pd.notna(atr_data):
            try:
