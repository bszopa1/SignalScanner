...
# --- Funktion für Signale mit RSI-Check ---
def compute_signals(df, rsi_len=14):
    ...
    df['Signal'] = ''
    for i in df.index:
        if pd.notna(df.at[i, 'SMA20']) and pd.notna(df.at[i, 'SMA50']):
            if df.at[i, 'SMA20'] > df.at[i, 'SMA50']:
                if pd.notna(df.at[i, 'rsi']) and df.at[i, 'rsi'] < 70:
                    df.at[i, 'Signal'] = 'BUY (RSI ok)'
                else:
                    df.at[i, 'Signal'] = 'BUY (RSI hoch)'
            elif df.at[i, 'SMA20'] < df.at[i, 'SMA50']:
                if pd.notna(df.at[i, 'rsi']) and df.at[i, 'rsi'] > 30:
                    df.at[i, 'Signal'] = 'SELL (RSI ok)'
                else:
                    df.at[i, 'Signal'] = 'SELL (RSI niedrig)'
    return df
