...
# --- Funktion für Signale mit RSI-Check (MultiIndex + iloc Fix) ---
def compute_signals(df, rsi_len=14):
    if 'Close' not in df.columns:
        return pd.DataFrame()

    # MultiIndex Check: Nur die erste Close-Spalte verwenden
    if isinstance(df['Close'], pd.DataFrame):
        df['Close'] = pd.to_numeric(df['Close'].iloc[:, 0], errors='coerce')
    else:
