...
def load_data(symbol):
    ...
    # MultiIndex prüfen (kommt z.B. bei auto_adjust=True)
...
def compute_signals(df, rsi_len=14):
    ...
    # Mindestens 20 Punkte für SMA20
    if len(df) < 20:
        return pd.DataFrame()
    ...
    # ATR berechnen, falls High/Low vorhanden
...
for name, df in all_data.items():
    ...
    # --- Signal extrahieren ---
    signal = last['Signal'] if pd.notna(last['Signal']) else None
