...
for name, df in all_data.items():
    ...
    # --- Signal robust extrahieren ---
    sig_value = last["Signal"]
    if isinstance(sig_value, (pd.Series, list, tuple)):
        sig_value = sig_value.iloc[0] if hasattr(sig_value, "iloc") else sig_value[0]

    signal = str(sig_value) if sig_value is not None else "N/A"

    stop_loss = take_profit = position_size = None

    if signal in ("BUY", "SELL") and not pd.isna(atr):
        atr = float(atr)
