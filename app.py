...
def load_data(ticker, period):
    ...
    # MultiIndex abflachen, falls vorhanden
...
def compute_signals(df, rsi_len=14):
    ...
    for pos in range(len(df)):
        ...
        # Falls Series, nur den ersten Wert nehmen
        if isinstance(close_val, pd.Series):
            close_val = float(close_val.iloc[0])
        if isinstance(sma20_val, pd.Series):
            sma20_val = float(sma20_val.iloc[0])
        if isinstance(sma50_val, pd.Series):
            sma50_val = float(sma50_val.iloc[0])
        if isinstance(rsi_val, pd.Series):
            rsi_val = float(rsi_val.iloc[0])

        # --- Kurzfrist (weniger als 50 Datenpunkte oder SMA50 fehlt)
        if len(df) < 50 or sma50_val is None:
            if sma20_val is not None:
                df.iloc[pos, df.columns.get_loc('Signal')] = 'BUY (Short)' if close_val > sma20_val else 'SELL (Short)'
