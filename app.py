...
# --- Funktion für Signale mit RSI-Check (iloc-fix) ---
def compute_signals(df, rsi_len=14):
    ...
    # Signale mit RSI (iloc statt at)
    df['Signal'] = ''
    for pos in range(len(df)):
        sma20 = df.iloc[pos]['SMA20']
        sma50 = df.iloc[pos]['SMA50']
        rsi = df.iloc[pos]['rsi']

        if pd.notna(sma20) and pd.notna(sma50):
            if sma20 > sma50:
                if pd.notna(rsi) and rsi < 70:
                    df.iloc[pos, df.columns.get_loc('Signal')] = 'BUY (RSI ok)'
                else:
                    df.iloc[pos, df.columns.get_loc('Signal')] = 'BUY (RSI hoch)'
            elif sma20 < sma50:
                if pd.notna(rsi) and rsi > 30:
                    df.iloc[pos, df.columns.get_loc('Signal')] = 'SELL (RSI ok)'
                else:
                    df.iloc[pos, df.columns.get_loc('Signal')] = 'SELL (RSI niedrig)'
    return df
